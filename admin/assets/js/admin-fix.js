...
function exportToCsv(data) {
  const headers = ['#', 'Username', 'Phone', 'Course', 'Events', 'Team Sizes', 'Event Count', 'Amount', 'Payment', 'College', 'IP', 'Token No', 'Created'];
  const csvContent = [
    headers.join(','),
    ...data.map((row, index) => {
      // Extract event information with team sizes
      let eventsStr = '';
      let teamSizesStr = '';
      ...
      try {
        ...
        // Check different possible event storage locations
        if (row.eventSelections && Array.isArray(row.eventSelections)) {
          eventsArray = row.eventSelections;
        } else if (row.events && Array.isArray(row.events)) {
          eventsArray = row.events;
        } else if (row.selectedEvents && Array.isArray(row.selectedEvents)) {
          eventsArray = row.selectedEvents;
        }
        
        if (eventsArray.length > 0) {
          const eventNames = eventsArray.map(ev => {
            const name = ev.eventTitle || ev.eventId || ev.name || ev.title || ev;
            return String(name);
          });
          
          const teamSizes = eventsArray.map(ev => {
            const size = ev.participantCount || ev.quantity || ev.teamSize || 1;
            return String(size);
          });
          
          eventsStr = eventNames.join('; ');
          teamSizesStr = teamSizes.join('; ');
        }
      } catch (e) {
        console.warn('Error extracting events for CSV:', e);
      }
  ...
  link.setAttribute('download', `evoka-registrations-${new Date().toISOString().split('T')[0]}.csv`);
...
async function runDiagnostics() {
  ...
  add('Firebase Connection', async () => {
    try {
      ...
      const { collection, getDocs, limit, query } = await import("https://www.gstatic.com/firebasejs/12.4.0/firebase-firestore.js");
